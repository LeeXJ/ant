...
// 1. 定义了命名空间 `ecs`，包含了一系列与实体组件系统相关的功能。
// 2. 定义了组件标识符和类型，以及用于判断组件属性的模板变量。
// 3. 提供了实体管理类 `context`，用于管理 ECS 的上下文，包括创建、获取、移除实体以及获取组件等操作。
// 4. 实现了实体选择器类 `selector` 和 `cached_selector`，用于选择和迭代实体。
// 5. 提供了一系列函数，用于清除特定类型的组件、启用组件组、统计组件数量等操作。