...
// 这是一个实现 SHA-1 哈希算法的 C 语言代码。以下是其主要功能：

// 1. 实现了 SHA-1 哈希算法的各种操作，包括初始化、更新和生成哈希值。
// 2. 包含了用于 SHA-1 计算的常量和宏定义。
// 3. 提供了对外的接口函数，用于初始化、更新数据和生成哈希值。
// 4. 包含了一些修正和优化，如在不同平台上的运行兼容性修正、消除编译警告、改进算法性能等。
// 5. 提供了测试向量，用于验证算法的正确性。
// 6. 最终通过对外的函数 `sat_SHA1_Final` 生成哈希值，将结果存储在给定的字节数组中。